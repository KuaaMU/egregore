...
(function() {
  ...
  function getResponse() {
    // Qwen uses .qk-markdown for responses
    const els = document.querySelectorAll('.qk-markdown, [class*="answer-common-card"], [class*="markdown"]');
    if (els.length === 0) return '';
    const text = els[els.length - 1].textContent?.trim() || '';
    // Filter out JSON garbage
  ...
  window.egregore = {
    platform: 'qwen', getInput, getResponse, isStreaming, sendPrompt, waitForResponse,
    getUrl: () => window.location.href, getConversationUrl: () => window.location.href,
    isReady: () => !!getInput(),
  };
  console.log('[Egregore] Qwen adapter loaded');
})();
